...
def hyperopt_obj(model_param, model_type, feat, trials_counter):
    ...
    if config.nthread == 1: # single process
        ...
        for mp in mp_list:
            print "Child Process %s, is_alive %s" %(mp.pid, mp.is_alive())
...
class ModelProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        with open("%s/valid.%s.feat.pkl" %(path, self.feat), 'rb') as f:
            [x_test, y_test] = pickle.load(f)
        f.close()
        pred_val = hyperopt_library(self.model_type, self.model_param, x_train, y_train, x_test)
        #pred_val = y_test
        ...
        print "Child Process %s done!!!" %os.getpid()
...
def hyperopt_main():
    ...
    for feat in feat_names:
        for model in model_list:
            ...
            print best_params
            param_best_dic["%s_%s"%(feat, model)] = best_params
